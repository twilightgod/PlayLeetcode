    ...
    public class Solution
    {
        public double FindMedianSortedArrays(int[] nums1, int[] nums2)
        {
            ...
            while (l < r)
            {
                ...
                var nums3_left = Math.Max(nums1_left, nums1_right);
                var nums3_right = Math.Min(nums2_left, nums2_right);

                // meet condition, = is for identical elements
                if (nums3_left <= nums3_right)
                {
                    if ((n3 & 1) == 1)
                    {
                    ...
                    else
                    {
                        ...
                        return (nums3_left + nums3_right) / 2.0;
                    }
                }
